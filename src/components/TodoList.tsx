...
//     const todoApp = {
//       id: Math.trunc(Math.random(10) * 100),
//       text: text,
//       completed: completed,
//     };
//     addTodo(todoApp);
...
function TodoList() {
  ...
  const [completed, setCompleted] = useState<boolean>(false);
  const initialRef = useRef<HTMLInputElement>(null);
  const [editId, setEditId] = useState<string | null>(null); // Changed editId type to string

  const add = async (text: string): Promise<void> => {
    const todoApp = {
      id: Math.random().toString(), // Generating a unique string ID
      text: text,
      completed: completed,
    };
    addTodo(todoApp);
  ...
  const toggleTodo = async (id: string): Promise<void> => {
    // Changed toggleTodo parameter type to string
    const todo = todos.find((todo) => todo?.id === id);
    if (todo) {
      onOpen();
      setEditText(todo.text);
  ...
  const handleCheck = (todo: Todo): void => {
    ...
    updateTodo(updatedTodo.id, updatedTodo); // Corrected parameter to updatedTodo.id
  };

  const deleteBtn = (id: string): void => {
    // Changed deleteBtn parameter type to string
    deleteTodo(id);
    toast({
      title: `Deleted successfully`,
      status: "error",
